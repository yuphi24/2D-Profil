...
// add MeasuredDepth-Data in the Chart
const addMDData = async () => {
  ...
  if (data.features.length > 0) {
    try {
      ...
      const newData = points.map((item) => ({
        ...
        y: item.properties.total_depth_MD,
...
// to allow parent component to use function addMDData
defineExpose({
  addMDData,
});
